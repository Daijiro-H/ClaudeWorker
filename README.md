...
Yahooは米国市場のクローズ直後に当日の日足を必ず公開するわけではなく、朝の実行時点で
まだ反映されていないことがあります。実際に2026-09-03の実行は、前日と同一の
2026-09-01のデータをそのまま配信しました(取得できた最終行を無検証で使っていたため)。

現在は `scripts/market.py` が鮮度を検証します。

- 米国東部時間で16:00のクローズを過ぎた直近の平日を「本来あるべきセッション」として算出します。
- 取得した最新の日足がそれより古い場合、45秒間隔で最大3回まで再取得します。
- それでも古い場合は**古い値を黙って配信せず**、通知の件名に `※データ遅延`、
  本文に未反映のセッション日を明記します。
...
```
※データ遅延: 2026-09-02 のセッションが未反映のため、2026-09-01 時点の値です(米国市場の休場日の場合もあります)
```
